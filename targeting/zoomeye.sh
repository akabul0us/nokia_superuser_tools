...
if ! curl -m 10 -s https://www.zoomeye.ai > /dev/null 2&>1; then
    zoomeye_status="$?"
	echo "It appears that Zoomeye's servers are down..."
	echo "Pinging IP address 154.93.109.29..."
	if ! ping -w 10 154.93.109.29 > /dev/null 2&>1; then
        ping_status="$?"
...
printf "${red}Title: GPON Home Gateway\n"
printf "${yellow}Port: 443\n"
printf "${green}jQuery 1.12\n${clear_color}"
if	[ ! -z $first_arg ]; then
	printf "${yellow}$(urlencode -d $first_arg)${clear_color}\n"
fi
